...
import { useRenewalRequests, useCreateRenewalRequest } from "@/hooks/useRenewals";
import { 
  CreditCard, 
  Sparkles, 
  Clock, 
...
const PACKAGES = [
  { months: 3, price: 300000, label: "3 tháng", originalPrice: 300000, desc: "Gói Cơ Bản", popular: false },
  { months: 6, price: 540000, label: "6 tháng (Giảm 10%)", originalPrice: 600000, desc: "Gói Phổ Biến", popular: true },
  { months: 12, price: 960000, label: "12 tháng (Giảm 20%)", originalPrice: 1200000, desc: "Gói Tiết Kiệm", popular: false },
];
 
const SUPERADMIN_BANK_ID = process.env.NEXT_PUBLIC_SUPERADMIN_BANK_ID || "MB";
const SUPERADMIN_BANK_ACC = process.env.NEXT_PUBLIC_SUPERADMIN_BANK_ACC || "123456789";
const SUPERADMIN_BANK_NAME = process.env.NEXT_PUBLIC_SUPERADMIN_BANK_NAME || "PLATFORM OWNER";
                ...
                <tbody className="divide-y divide-gray-50 text-gray-700">
                  {requests.map((req) => (
                    <tr key={req.id} className="hover:bg-gray-50/50">
                      ...
                      <td className="px-6 py-4 font-bold">{req.months} tháng</td>
            ...
            <div className="p-6 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-lg text-gray-900">
                  Gia hạn Premium {selectedPkg.months} tháng
                </h3>
              </div>
              <button 
                onClick={() => {
            ...
            <div className="p-6 space-y-6 max-h-[75vh] overflow-y-auto">
              <div className="bg-orange-50 border border-orange-100 rounded-2xl p-4 flex flex-col items-center gap-3">
                ...
                <div className="w-44 h-44 bg-white border border-gray-200/60 rounded-xl flex items-center justify-center p-2 relative shadow-md">
                  <img
                    src={`https://img.vietqr.io/image/${SUPERADMIN_BANK_ID}-${SUPERADMIN_BANK_ACC}-compact2.png?amount=${selectedPkg.price}&addInfo=GIAHAN%20${storeConfig?.slug}%20${selectedPkg.months}&accountName=${encodeURIComponent(SUPERADMIN_BANK_NAME)}`}
                    alt="VietQR code"
              ...
              <div className="bg-gray-50 rounded-2xl p-4 text-xs font-semibold space-y-2 text-gray-700">
                <div className="flex justify-between border-b border-gray-200/50 pb-1.5">
                  ...
                  <span className="font-bold text-gray-800">{SUPERADMIN_BANK_ID}</span>
                </div>
                <div className="flex justify-between border-b border-gray-200/50 pb-1.5 items-center">
                  ...
                  <div className="flex items-center gap-1.5">
                    <span className="font-bold text-gray-800">{SUPERADMIN_BANK_ACC}</span>
                    <button 
                      onClick={() => handleCopy(SUPERADMIN_BANK_ACC, "Số tài khoản")}
                ...
                <div className="flex justify-between border-b border-gray-200/50 pb-1.5">
                  ...
                  <span className="font-bold text-gray-800">{SUPERADMIN_BANK_NAME}</span>
                </div>
                <div className="flex justify-between border-b border-gray-200/50 pb-1.5 items-center">
                  <span className="text-gray-400 font-bold">Nội dung chuyển khoản</span>
                  <div className="flex items-center gap-1.5">
                    <span className="font-bold text-orange-600">GIAHAN {storeConfig?.slug} {selectedPkg.months}</span>
                    <button 
                      onClick={() => handleCopy(`GIAHAN ${storeConfig?.slug} ${selectedPkg.months}`, "Nội dung chuyển khoản")}
                      className="text-primary hover:text-orange-600 transition-colors"
                    >
                      <Copy size={12} />
                    </button>
